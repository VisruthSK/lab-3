...
<!-- 
TODO: 
1. Adjust page titles
2. Adjust colours to all match with theme
3. Add Flash Facts
4. Make first column wider
-->
...
library(gt)
use("glue", "glue")
use("scales", c("label_percent", "percent"))
...
vaccine |>
  # ungroup() |>
  # mutate(Region = fct_reorder(Region, safe)) |>
  # group_by(Region) |>
  arrange(safe) |>
  gt() |>
...
```{r funky plot}
#| title: Proportion of people who believed vaccines are safe.
library(gghalves)
